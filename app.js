...
// Servir el archivo HTML desde la ruta /diagnosis
app.get('/diagnosis', (req, res) => {
    res.sendFile(path.join(__dirname, 'ui', 'diagnosis', 'index.html'));
});

// Servir la ruta /diagnosis-data para los datos de diagnóstico
app.get('/diagnosis-data', async (req, res) => {
    try {
        const results = await Promise.all(Object.entries(scrappers).map(async ([site, url]) => {
            let status = 'red';
            let message = 'Unable to reach the page';
            let screenshot = null;
            ...
            try {
                ...
                message = `Puppeteer launch error: ${err.message}`;
            } finally {
                if (browser) {
                    await browser.close();
                }
            ...
            return { site, status, message, screenshot };
        }));
